      ...
      if (document.getElementById('entity-target').value === "website") {
        if ((el.getAttribute('class') !== null && el.getAttribute('class').indexOf('capFL') !== -1) || (el.parentNode.getAttribute('class') !== null && el.parentNode.getAttribute('class').indexOf('capFL') !== -1)) {
          el.innerHTML = "O sítio Web";
			...
			if (el.getAttribute('class') !== null && el.getAttribute('class').indexOf('nType') !== -1) {
				el.innerHTML = "ao sítio Web";
			} else {
				el.innerHTML = "o sítio Web";
			}       
        }
       ...
       // if ((el.getAttribute('class') !== null && el.getAttribute('class').indexOf('capFL') === -1) || (el.parentNode.getAttribute('class') !== null && el.parentNode.getAttribute('class').indexOf('capFL') === -1)) {
		...
		if ((el.getAttribute('class') !== null && el.getAttribute('class').indexOf('capFL') !== -1) || (el.parentNode.getAttribute('class') !== null && el.parentNode.getAttribute('class').indexOf('capFL') !== -1)) {
          el.innerHTML = "A aplicação móvel";
			...
			if (el.getAttribute('class') !== null && el.getAttribute('class').indexOf('nType') !== -1) {
				el.innerHTML = "à aplicação móvel";
			} else {
				el.innerHTML = "a aplicação móvel";
			}
        }
          ...
          default:
            if (item.id === "accstmnt_assessment_with_tools_summary" && printData) {
              ...
              if (printData !== "no") {
                ...
                outHTML += "</p>"
                outHTML += "<ul>";
                outHTML += '<li>passa a lista de verificação “Conteúdos”</li>';
              ...
              if (printData === "Prata" || printData === "Ouro") {
                outHTML += '<li>passa a lista de verificação “Transação”</li>';
                outHTML += '<li>passa a lista de verificação “10 aspetos funcionais”</li>';
              }
              if (printData === "Ouro") {
